...
canvas.title("Shakespeare Write")
canvas.config(bg = "White")
top = tk.Frame(canvas)
top.pack(padx = 10, pady = 5, anchor = 'nw')

def saveFile():
    new_file = asksaveasfile(mode = 'w', filetype = [('text files', '.txt'), ('Python file', '.py')])
    if new_file is None:
        return
...
def invertFile():
    ...
    entry.config(bg = "#004d80", fg = "#ffffff")
...
b4 = tk.Button(canvas, text="Invert", bg = "White", command = invertFile)
b4.pack(in_ = top, side=tk.LEFT, )

entry = tk.Text(canvas, wrap = tk.WORD, bg = "#F9DDA4", font = ("arial", 15))
